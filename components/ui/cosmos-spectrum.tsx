...
interface CosmicSpectrumProps {
  color?:
    ...
    | "beige-black"
  blur?: boolean
  /** Hero title text. Defaults to "Cosmos Spectrum". */
  title?: string
  /** Optional small line shown under the title. */
  subtitle?: string
  /** Bottom-center scroll hint. Defaults to "Scroll to explore". */
  scrollHint?: string
  /** Override the title sizing/weight/font. */
      ...
      <section className="h-screen w-full p-8 flex flex-col items-center justify-center relative">
        <h1 className={`hero-title text-center transition-colors duration-300 ${titleClassName}`}>
          <LetterSwapPingPong
            label={title}
            staggerFrom="center"
            className="cursor-pointer leading-[1.35]"
          />
        </h1>
        {subtitle && (
          <p className="hero-text font-neutrek mt-1 text-center text-[0.65rem] uppercase tracking-[0.35em] opacity-70 sm:text-xs">
            {subtitle}
          </p>
        )}
      </section>
